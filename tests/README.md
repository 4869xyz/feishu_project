...
| `test_feishu_bot_listener.py` | Wiki 权限提示、群聊提及、日志安全、固定云表数字编号/排序/批量清空与刷新、汇总、缓存清理和文件回复。 |
| `test_sales_workbook_aggregator.py` | 单/多工作表选表优先级、签约校验、跨文件非去重、源字体颜色、C:G 暖色隐藏、I:T 会计专用一位小数与自适应列宽、模板样式、汇总区块空行、公式、控制总额、回款隔离和非目标表保留。 |
