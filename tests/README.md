...
| `test_feishu_bot_listener.py` | Wiki 权限提示、群聊提及、日志安全、单实例锁、汇总命令、缓存清理权限和文件回复。 |
| `test_sales_workbook_aggregator.py` | 单/多工作表选表优先级、签约校验、跨文件非去重、公式、控制总额、回款隔离和非目标表保留。 |
| `test_aggregation_batch_store.py` | 批次持久化、上传顺序、来源去重、发送人隔离、全局活动文件查询和输出路径。 |
