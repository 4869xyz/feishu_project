...
$exePath = Join-Path $programRoot "MeetingMinutesBot.exe"
$logsRoot = Join-Path $programRoot "logs\meeting_minutes"
$pidPath = Join-Path $logsRoot "meeting_bot_launcher.pid"
...
try {
    ...
    # 首次启动需要加载本地 OCR 模型，等待时间比销售机器人更长。
