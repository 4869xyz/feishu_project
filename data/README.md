...
| `data/meeting_minutes/meeting_minutes.db` | 第二机器人独立的事件、提交、提醒轮次和纪要版本数据库。 | 整个目录忽略。 |
...
| `data/meeting_minutes/submission_docs/` | 用户提交的源 Word 副本（含表格/图片），生成周报时原样嵌入；随保留期清理。 | 整个目录忽略。 |
...
配置过管理员 `open_id` 后，可向机器人发送 `清空下载缓存`，删除 `inbox/`、`archive/` 和 `aggregation/output/` 中未被活动批次引用的文件。该命令不会扫描或删除 `aggregation/state/`、`aggregation/registered/`、汇总模板、活动批次源文件或 `.gitkeep`；单个文件删除失败时会继续处理其余文件，并在回复中报告失败数。
