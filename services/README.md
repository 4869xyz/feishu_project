...
汇总引擎的调用者必须为每个源文件提供稳定且唯一的 `source_file_id`。该 ID 只用于防止同一飞书来源重复加入批次，不用于业务内容去重。引擎会解析静态 RGB、主题色和索引字体色；人员至客户来源字段 C:G 任一字体为黄色/橙色时隐藏该明细，其他列颜色不参与判定，隐藏数据仍参与所有汇总公式。生成的 I:T 明细和汇总金额统一使用无货币符号、1 位小数的会计专用数字格式，并按每个月份金额长度独立扩展列宽。
