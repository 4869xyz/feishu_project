...
- 可用 `添加云表 <链接>` 长期登记固定 Sheets/Wiki 来源；执行 `汇总` 时会先顺序刷新最新 XLSX，失败时不会回退旧缓存；
...
云表列表
移除云表 cloud-xxxxxxxxxxxx
```

登记时会立即导出并校验销售表结构，成功后将稳定编号、原始链接和 latest 缓存记录在当前“会话 + 发送人”的状态中。Wiki 来源在每次刷新时都会重新解析真实对象。固定云表缓存位于：
...
2. 发送 `汇总状态` 查看固定云表和临时文件；发送 `清空汇总` 只清除临时批次，固定云表继续保留。
3. 发送 `汇总`。机器人先按登记顺序重新导出并校验所有固定云表，再将其放在临时文件之前参与汇总。任一固定来源权限失效、删除、超时或校验失败时整体中止，不使用旧缓存。
4. 机器人以 `FEISHU_SALES_TEMPLATE_PATH` 为模板，只清空并重建签约目标表的明细、统计行、合并单元格和公式，然后把新的结果 Excel 发回会话。签约明细 A:T 逐格保留源表字体颜色，字体名称、字号、粗体、填充、边框、对齐、行高和数字格式仍使用模板；回款表和其他模板工作表原样保留。
...
当前完整基线：**88 项测试通过**。此外，汇总结果会通过 Microsoft Excel 兼容重开和可视检查验证签约公式、源字体颜色与模板布局。
