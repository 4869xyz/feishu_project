...
4. 机器人以 `FEISHU_SALES_TEMPLATE_PATH` 为模板，只清空并重建签约目标表的明细、统计行、合并单元格和公式，然后把新的结果 Excel 发回会话。回款表和其他模板工作表原样保留。
...
当前完整基线：**85 项测试通过**。此外，汇总结果会通过 Microsoft Excel 兼容重开和可视检查验证签约公式与模板布局。
