...
- 可用 `添加云表 <链接>` 长期登记固定 Sheets/Wiki 来源，并通过连续数字编号进行查看、删除、排序和批量清空；执行 `汇总` 时会先顺序刷新最新 XLSX，失败时不会回退旧缓存；
...
登记时会立即导出并校验销售表结构。`云表列表` 按当前汇总顺序显示连续编号 `1…N`；删除或排序后会自动重新编号。`云表排序` 必须完整填写当前所有编号且不能重复，例如三份云表可发送 `云表排序 3 1 2`。`清空云表` 只清除当前“会话 + 发送人”的全部固定云表及其 latest 缓存，不影响临时上传文件。
...
1. 使用 `添加云表 <链接>` 登记长期来源；临时文件仍可依次上传附件或普通发送表格链接。所有来源在加入时必须通过 A:T 销售表结构校验。
...
4. 机器人以 `FEISHU_SALES_TEMPLATE_PATH` 为模板，只清空并重建签约目标表的明细、统计行、合并单元格和公式，然后把新的结果 Excel 发回会话。签约明细 A:T 逐格保留源表字体颜色；人员、客户简称、产品/项目、落地地点或客户来源（C:G）任一字段使用黄色/橙色字体时自动隐藏整条明细，但数据及汇总计算仍保留。I:T 明细和汇总金额统一使用无货币符号、1 位小数的会计专用格式，并按各月份金额显示长度自适应列宽；字体名称、字号、粗体、填充、边框、对齐和行高仍使用模板。回款表和其他模板工作表原样保留。
...
当前完整基线：**97 项测试通过**。此外，汇总结果会通过 Microsoft Excel 兼容重开和可视检查验证签约公式、源字体颜色、关键字段黄色/橙色明细隐藏、金额格式、自适应列宽与模板布局。
